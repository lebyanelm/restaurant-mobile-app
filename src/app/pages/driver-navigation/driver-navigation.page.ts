...
import { Component, ViewChild, AfterViewInit, ErrorHandler } from '@angular/core';
...
export class DriverNavigationPage implements AfterViewInit {
  @ViewChild('SlideButton', {static: false}) slideButton: SlideButtonComponent;
      ...
      if (this.deliveries.length === 3) {
        ...
        this.locationDispatchInterval = setInterval(() => {
          Plugins.Geolocation.getCurrentPosition({ enableHighAccuracy: true })
            .then((location) => {
              const coordinates = { lat: location.coords.latitude, lng: location.coords.longitude, speed: location.coords.speed };
  ...
  sendOrderStatusUpdate(self: DriverNavigationPage, orderIndex: number): void {
    post(environment.BACKEND + 'order/status')
      ...
      .send({ orderIds: [self.deliveries[orderIndex].id], status: 4, branchId: this.activeBranch.id })
  ...
  selectBranch() {
    if (this.deliveries.length === 0) {
      get(environment.BACKEND + 'branches?token=' + this.data.token)
      .end((error, response) => {
        if (!error) {
      ...
      this.toast.showAlert({
        ...
        message: 'You currently have active deliveries with the connected branch.' });
    }
  }
  ...
  signOut() {
    ...
    this.storage.remove(environment.customerDataName)
      .then(() => {
        this.storage.remove(environment.driverDataName)
          .then(() => {
    ...
    this.sockets.connection.emit('set status', {
      ...
      if (response.isSet) {
        ...
        if (errorHandler) {
          ...
          this.toast.show('Error while changing connection status, please try again. Contact us if error persists.');
        }
      }
    });
  }
