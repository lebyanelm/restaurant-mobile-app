...
export class ForgotPasswordPage implements AfterViewInit {
  ...
  constructor(private router: Router, private activatedRoute: ActivatedRoute, private storage: StorageService, private sockets: SocketsService) { }
    ...
    fields.forEach((field, index) => {
      ...
      input.onkeyup = () => {
        if (index === this.currentSignupStep) {
          if (index === 0) {
            // tslint:disable-next-line: max-line-length
            this.check(!((input.value.startsWith('0') && input.value.length !== 10) || (!input.value.startsWith('0') && input.value.length !== 12)));
          } else if (index === 1) {
      ...
      .end((_, response) => {
        if (response) {
          if (response.status === 200) {
            ...
            this.instructions.nativeElement.innerText = 'An SMS has been sent to your phone number.';
            this.nextButtonText.nativeElement.innerText = 'Verify';
            this.nextButton.nativeElement.onclick = () => { this.verifyPhoneNumber(); };
            this.nextSlide();
          } else if (response.status === 404) {
            this.isLoading = false;
            this.instructions.nativeElement.innerText = 'An account with that phone number was not found. Please consider creating an account.';
          } else {
            this.isLoading = false;
            this.instructions.nativeElement.innerText = response.body.reason ? response.body.reason : 'There was an unexpected error sending the SMS, try again. Let us know if the problem persists.';
          }
        } else {
          this.isLoading = false;
          this.instructions.nativeElement.innerHTML = 'No internet connection, please check your internet connection and try again.';
        }
      });
  }
      ...
      .end((error, response) => {
        ...
        if (!error) {
          if (response.status === 200) {
            ...
            this.isError = true;
            this.instructions.nativeElement.innerText = 'Invalid verification code entered. Make sure it\'s typed correctly.';
          }
        } else {
          if (response === undefined) {
            this.isError = true;
            this.isLoading = false;
            this.instructions.nativeElement.innerText = 'There was a server connection error. Check your internet connection and try again.';
          }
        }
      });
  }
        ...
        if (!error) {
          ...
          if (response) {
            if (response.status === 500) {
              ...
              this.instructions.nativeElement.innerText = 'An unexpected error has occured. Please try again.' + (code.length > 0 ? 'Reference code: ' + code : '');
