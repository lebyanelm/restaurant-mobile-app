  ...
  login(): void {
    ...
    superagent
      .get(
        [
          environment.BACKEND,
          'customers?partnerId=' + environment.PARTNER_ID,
        ].join('')
      )
      ...
      .end((_, response) => {
        if (response.status === 200) {
          ...
          this.storage
            ...
            .then(() => {
              ...
              if (response.body.isDriver) {
                this.storage
                  .setItem(environment.driverDataName, {
                ...
                _self.sockets
                  ...
                  .then(() => {
                    ...
                    _self.router.navigate(
                      response.body.isDriver ? ['driver-navigation'] : ['home']
                    );
                  })
                  .catch(() => {
                    _self.error =
                      'An error occured while trying to establish a connection. Please try again, let us know if problem persists.';
                  });
              }
            });
          ...
          this.isLoading = false;
          this.error =
            'Something wrong happened, let us know if this persists.';
        }
      });
  }
}
